...
public final class Lucene84OrdsSegmentTermsEnum extends BaseTermsEnum {
    ...
    private Lucene84OrdsSegmentTermsEnumFrame[] stack;
    private final Lucene84OrdsSegmentTermsEnumFrame staticFrame;
    Lucene84OrdsSegmentTermsEnumFrame currentFrame;
    boolean termExists;
    final Lucene84OrdsFieldReader fr;
    ...
    Lucene84OrdsSegmentTermsEnum(Lucene84OrdsFieldReader fr) throws IOException {
        this.fr = fr;

        //if (DEBUG) System.out.println("BTTR.init seg=" + segment);
        stack = new Lucene84OrdsSegmentTermsEnumFrame[0];

        // Used to hold seek by TermState, or cached seek
        staticFrame = new Lucene84OrdsSegmentTermsEnumFrame(this, -1);
    ...
    private Lucene84OrdsSegmentTermsEnumFrame getFrame(int ord) throws IOException {
        if (ord >= stack.length) {
            final Lucene84OrdsSegmentTermsEnumFrame[] next = new Lucene84OrdsSegmentTermsEnumFrame[ArrayUtil.oversize(1+ord, RamUsageEstimator.NUM_BYTES_OBJECT_REF)];
            System.arraycopy(stack, 0, next, 0, stack.length);
            for(int stackOrd=stack.length;stackOrd<next.length;stackOrd++) {
                next[stackOrd] = new Lucene84OrdsSegmentTermsEnumFrame(this, stackOrd);
            }
            stack = next;
        }
    ...
    Lucene84OrdsSegmentTermsEnumFrame pushFrame(FST.Arc<Output> arc, Output frameData, int length) throws IOException {
        ...
        final Lucene84OrdsSegmentTermsEnumFrame f = getFrame(1+currentFrame.ord);
    ...
    Lucene84OrdsSegmentTermsEnumFrame pushFrame(FST.Arc<Output> arc, long fp, int length, long termOrd) throws IOException {
        final Lucene84OrdsSegmentTermsEnumFrame f = getFrame(1+currentFrame.ord);
        f.arc = arc;
    ...
    */
        ...
        if (positioned && currentFrame != staticFrame) {
            ...
            Lucene84OrdsSegmentTermsEnumFrame lastFrame = stack[0];
    ...
    private void printSeekState(PrintStream out) throws IOException {
        if (currentFrame == staticFrame) {
            ...
            while(true) {
                Lucene84OrdsSegmentTermsEnumFrame f = getFrame(ord);
                assert f != null;
    ...
    public String toString() {
        return "Lucene84OrdsSegmentTermsEnum(seg=" + fr.parent + ")";
    }
