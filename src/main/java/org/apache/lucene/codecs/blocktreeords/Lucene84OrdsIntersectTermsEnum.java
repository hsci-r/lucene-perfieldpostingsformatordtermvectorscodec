...
// NOTE: cannot seek!
final class Lucene84OrdsIntersectTermsEnum extends BaseTermsEnum {
    final IndexInput in;

    private Lucene84OrdsIntersectTermsEnumFrame[] stack;
    ...
    private Lucene84OrdsIntersectTermsEnumFrame currentFrame;
    ...
    public Lucene84OrdsIntersectTermsEnum(Lucene84OrdsFieldReader fr, CompiledAutomaton compiled, BytesRef startTerm) throws IOException {
        // if (DEBUG) {
        ...
        stack = new Lucene84OrdsIntersectTermsEnumFrame[5];
        for(int idx=0;idx<stack.length;idx++) {
            stack[idx] = new Lucene84OrdsIntersectTermsEnumFrame(this, idx);
        }
        ...
        final Lucene84OrdsIntersectTermsEnumFrame f = stack[0];
    ...
    private Lucene84OrdsIntersectTermsEnumFrame getFrame(int ord) throws IOException {
        if (ord >= stack.length) {
            final Lucene84OrdsIntersectTermsEnumFrame[] next = new Lucene84OrdsIntersectTermsEnumFrame[ArrayUtil.oversize(1+ord, RamUsageEstimator.NUM_BYTES_OBJECT_REF)];
            System.arraycopy(stack, 0, next, 0, stack.length);
            for(int stackOrd=stack.length;stackOrd<next.length;stackOrd++) {
                next[stackOrd] = new Lucene84OrdsIntersectTermsEnumFrame(this, stackOrd);
            }
            stack = next;
        }
    ...
    private Lucene84OrdsIntersectTermsEnumFrame pushFrame(int state) throws IOException {
        final Lucene84OrdsIntersectTermsEnumFrame f = getFrame(currentFrame == null ? 0 : 1+currentFrame.ord);
    ...
    // TODO: do we need ord() here?  Lucene84OrdsIntersectTermsEnumFrame tracks termOrd but it may be buggy!
